...
class TestProblem(BaseTest):
    ...
    def test_to_str(self):
        ...
        prob = Problem(cvx.Minimize(self.a), [Zero(self.a), NonPos(self.a)])
        self.assertEqual(str(prob), result)

    def test_variables(self):
        """Test the variables method.
        """
        p = Problem(cvx.Minimize(self.a), [self.a <= self.x, self.b <= self.A + 2])
        vars_ = p.variables()
    ...
    def test_parameters(self):
        ...
        p = Problem(cvx.Minimize(p1), [self.a + p1 <= p2, self.b <= p3 + p3 + 2])
        params = p.parameters()
        ref = [p1, p2, p3]
        if PY2:
    ...
    def test_size_metrics(self):
        ...
        p = Problem(cvx.Minimize(p1), [self.a + p1 <= p2,
    ...
    def test_solver_stats(self):
        ...
        """
        prob = Problem(cvx.Minimize(cvx.norm(self.x)), [self.x == 0])
    ...
    def test_get_problem_data(self):
        ...
        """
        data, _, _ = Problem(cvx.Minimize(cvx.exp(self.a) + 2)).get_problem_data(s.SCS)
        dims = data[ConicSolver.DIMS]
        self.assertEqual(dims.exp, 1)
        self.assertEqual(data["c"].shape, (2,))
        self.assertEqual(data["A"].shape, (3, 2))

        data, _, _ = Problem(cvx.Minimize(cvx.norm(self.x) + 3)).get_problem_data(s.ECOS)
        dims = data[ConicSolver.DIMS]
        ...
        if s.CVXOPT in INSTALLED_SOLVERS:
            import cvxopt
            data, _, _ = Problem(cvx.Minimize(cvx.norm(self.x) + 3)).get_problem_data(
                s.CVXOPT)
            dims = data[ConicSolver.DIMS]
            self.assertEqual(dims.soc, [3])
    ...
    def test_unpack_results(self):
        ...
        """
        prob = Problem(cvx.Minimize(cvx.exp(self.a)), [self.a == 0])
    ...
    #     Problem.register_solve("test", test)
    #     p = Problem(cvx.Minimize(obj), [eq, eq, le, le])
    #     result = p.solve(method="test")
    ...
    #     exp = norm(self.x, 2)
    #     prob = Problem(cvx.Minimize(0), [exp <= 1, exp <= 2])
    #     result = prob.solve(method="test")
    #     self.assertEqual(result, (0, 3))

    # Test the is_dcp method.
    def test_is_dcp(self):
        p = Problem(cvx.Minimize(cvx.norm_inf(self.a)))
        self.assertEqual(p.is_dcp(), True)

        p = Problem(cvx.Maximize(cvx.norm_inf(self.a)))
    ...
    def test_is_qp(self):
        ...
        obj = cvx.sum_squares(A*self.y - b)
        qpwa_obj = 3*cvx.sum_squares(-cvx.abs(A*self.y)) +\
            cvx.quad_over_lin(cvx.maximum(cvx.abs(A*self.y), [3.,3.,3.,3.]), 2.)
        not_qpwa_obj = 3*cvx.sum_squares(cvx.abs(A*self.y)) +\
            cvx.quad_over_lin(cvx.minimum(cvx.abs(A*self.y), [3.,3.,3.,3.]), 2.)

        p = Problem(cvx.Minimize(obj),[])
        self.assertEqual(p.is_qp(), True)

        p = Problem(cvx.Minimize(qpwa_obj),[])
        self.assertEqual(p.is_qp(), True)

        p = Problem(cvx.Minimize(not_qpwa_obj),[])
        self.assertEqual(p.is_qp(), False)

        p = Problem(cvx.Minimize(obj),[Aeq * self.y == beq, F * self.y <= g])
        self.assertEqual(p.is_qp(), True)

        p = Problem(cvx.Minimize(qpwa_obj),[Aeq * self.y == beq, F * self.y <= g])
        self.assertEqual(p.is_qp(), True)

        p = Problem(cvx.Minimize(obj),[cvx.maximum(1, 3 * self.y) <= 200, cvx.abs(2 * self.y) <= 100,
            cvx.norm(2 * self.y, 1) <= 1000, Aeq * self.y == beq])
        self.assertEqual(p.is_qp(), True)

        p = Problem(cvx.Minimize(qpwa_obj),[cvx.maximum(1, 3 * self.y) <= 200, cvx.abs(2 * self.y) <= 100,
            cvx.norm(2 * self.y, 1) <= 1000, Aeq * self.y == beq])
        self.assertEqual(p.is_qp(), True)

        p = Problem(cvx.Minimize(obj),[cvx.maximum(1, 3 * self.y ** 2) <= 200])
        self.assertEqual(p.is_qp(), False)

        p = Problem(cvx.Minimize(qpwa_obj),[cvx.maximum(1, 3 * self.y ** 2) <= 200])
        self.assertEqual(p.is_qp(), False)

    # Test problems involving variables with the same name.
    def test_variable_name_conflict(self):
        var = Variable(name='a')
        p = Problem(cvx.Maximize(self.a + var), [var == 2 + self.a, var <= 3])
        result = p.solve()
    ...
    def test_add_problems(self):
        prob1 = Problem(cvx.Minimize(self.a), [self.a >= self.b])
        prob2 = Problem(cvx.Minimize(2*self.b), [self.a >= 1, self.b >= 2])
        prob_minimize = prob1 + prob2
        self.assertEqual(len(prob_minimize.constraints), 3)
        self.assertAlmostEqual(prob_minimize.solve(), 6)
        prob3 = Problem(cvx.Maximize(self.a), [self.b <= 1])
        prob4 = Problem(cvx.Maximize(2*self.b), [self.a <= 2])
        ...
        prob5 = Problem(cvx.Minimize(3*self.a))
        ...
        # Test cvx.Minimize + cvx.Maximize
        with self.assertRaises(DCPError) as cm:
    ...
    def test_mul_problems(self):
        prob1 = Problem(cvx.Minimize(pow(self.a, 2)), [self.a >= 2])
        answer = prob1.solve()
    ...
    def test_lin_combination_problems(self):
        prob1 = Problem(cvx.Minimize(self.a), [self.a >= self.b])
        prob2 = Problem(cvx.Minimize(2*self.b), [self.a >= 1, self.b >= 2])
        prob3 = Problem(cvx.Maximize(-pow(self.b + self.a, 2)), [self.b >= 3])

        # simple addition and multiplication
        combo1 = prob1 + 2 * prob2
        combo1_ref = Problem(cvx.Minimize(self.a + 4 * self.b), [self.a >= self.b, self.a >= 1, self.b >= 2])
        self.assertAlmostEqual(combo1.solve(), combo1_ref.solve())

        # division and subtraction
        combo2 = prob1 - prob3/2
        combo2_ref = Problem(cvx.Minimize(self.a + pow(self.b + self.a, 2)/2), [self.b >= 3, self.a >= self.b])
        ...
        combo3_ref = Problem(cvx.Minimize(self.a + 3 * pow(self.b + self.a, 2)), [self.a >= self.b, self.a >= 1, self.b >= 3])
        self.assertAlmostEqual(combo3.solve(), combo3_ref.solve())

    # Test solving problems in parallel.
    def test_solve_parallel(self):
        p = Parameter()
        problem = Problem(cvx.Minimize(cvx.square(self.a) + cvx.square(self.b) + p),
                          [self.b >= 2, self.a >= 1])
        p.value = 1
        ...
        objective = cvx.Minimize(cvx.square(self.a) + cvx.square(self.b))
    ...
    def test_scalar_lp(self):
        p = Problem(cvx.Minimize(3*self.a), [self.a >= 2])
        result = p.solve()
        self.assertAlmostEqual(result, 6)
        self.assertAlmostEqual(self.a.value, 2)

        p = Problem(cvx.Maximize(3*self.a - self.b),
        ...
        p = Problem(cvx.Minimize(3*self.a - self.b + 100),
                    [self.a >= 2,
        ...
        # Unbounded problems.
        p = Problem(cvx.Maximize(self.a), [self.a >= 2])
        ...
        if s.CVXOPT in INSTALLED_SOLVERS:
            p = Problem(cvx.Minimize(-self.a), [self.a >= 2])
        ...
        # Infeasible problems.
        p = Problem(cvx.Maximize(self.a), [self.a >= 2, self.a <= 1])
        self.a.save_value(2)
        ...
        p = Problem(cvx.Minimize(-self.a), [self.a >= 2, self.a <= 1])
        result = p.solve(solver=s.ECOS)
        self.assertEqual(result, p.value)
        self.assertEqual(p.status, s.INFEASIBLE)
        assert numpy.isinf(p.value)
    ...
    def test_ecos_noineq(self):
        ...
        p = Problem(cvx.Minimize(1), [self.A == T])
        result = p.solve(solver=s.ECOS)
    ...
    def test_variable_promotion(self):
        p = Problem(cvx.Minimize(self.a), [self.x <= self.a, self.x == [1, 2]])
        result = p.solve()
        self.assertAlmostEqual(result, 2)
        self.assertAlmostEqual(self.a.value, 2)

        p = Problem(cvx.Minimize(self.a),
                    [self.A <= self.a,
                     self.A == [[1, 2], [3, 4]]
                     ])
        result = p.solve()
        ...
        p = Problem(cvx.Minimize([[1], [1]]*(self.x + self.a + 1)),
                    [self.a + self.x >= [1, 2]])
        result = p.solve()
        self.assertAlmostEqual(result, 5)
    ...
    def test_norm_inf(self):
        # Constant argument.
        p = Problem(cvx.Minimize(cvx.norm_inf(-2)))
        result = p.solve()
        self.assertAlmostEqual(result, 2)

        # Scalar arguments.
        p = Problem(cvx.Minimize(cvx.norm_inf(self.a)), [self.a >= 2])
        result = p.solve()
        self.assertAlmostEqual(result, 2)
        self.assertAlmostEqual(self.a.value, 2)

        p = Problem(cvx.Minimize(3*cvx.norm_inf(self.a + 2*self.b) + self.c),
        ...
        # cvx.Maximize
        p = Problem(cvx.Maximize(-cvx.norm_inf(self.a)), [self.a <= -2])
        result = p.solve()
        self.assertAlmostEqual(result, -2)
        self.assertAlmostEqual(self.a.value, -2)
    ...
    def test_norm1(self):
        # Constant argument.
        p = Problem(cvx.Minimize(cvx.norm1(-2)))
        result = p.solve()
        self.assertAlmostEqual(result, 2)

        # Scalar arguments.
        p = Problem(cvx.Minimize(cvx.norm1(self.a)), [self.a <= -2])
        result = p.solve()
        self.assertAlmostEqual(result, 2)
        self.assertAlmostEqual(self.a.value, -2)

        # cvx.Maximize
        p = Problem(cvx.Maximize(-cvx.norm1(self.a)), [self.a <= -2])
        result = p.solve()
        self.assertAlmostEqual(result, -2)
        self.assertAlmostEqual(self.a.value, -2)

        # Vector arguments.
        p = Problem(cvx.Minimize(cvx.norm1(self.x - self.z) + 5),
                    [self.x >= [2, 3], self.z <= [-1, -4]])
        result = p.solve()
    ...
    def test_abs(self):
        p = Problem(cvx.Minimize(cvx.sum(cvx.abs(self.A))), [-2 >= self.A])
        result = p.solve()
    ...
    def test_quad_form(self):
        ...
        with self.assertRaises(Exception) as cm:
            Problem(cvx.Minimize(cvx.quad_form(1, self.A))).solve()
        ...
        c = [1, 2]
        p = Problem(cvx.Minimize(cvx.quad_form(c, self.A)), [self.A >= 1])
        result = p.solve()
        self.assertAlmostEqual(result, 9)

        c = [1, 2]
        P = [[4, 0], [0, 9]]
        p = Problem(cvx.Minimize(cvx.quad_form(c, P)))
        result = p.solve()
    ...
    def test_indexing(self):
        ...
        x = Variable((n, n))
        p = Problem(cvx.Minimize(cvx.sum(x)), [x == A])
        result = p.solve()
        answer = n*n*(n*n+1)/2 - n*n
        self.assertAlmostEqual(result, answer)

        # Matrix variables
        p = Problem(cvx.Maximize(sum(self.A[i, i] + self.A[i, 1-i] for i in range(2))),
                    [self.A <= [[1, -2], [-3, 4]]])
        result = p.solve()
    ...
    def test_non_python_int_index(self):
        ...
        p = Problem(cvx.Minimize(cost), [self.x == 1])
        result = p.solve()
        self.assertAlmostEqual(result, 1)
    ...
    def test_slicing(self):
        p = Problem(cvx.Maximize(cvx.sum(self.C)), [self.C[1:3, :] <= 2, self.C[0, :] == 1])
        result = p.solve()
        self.assertAlmostEqual(result, 10)
        self.assertItemsAlmostEqual(self.C.value, 2*[1, 2, 2])

        p = Problem(cvx.Maximize(cvx.sum(self.C[0:3:2, 1])),
                    [self.C[1:3, :] <= 2, self.C[0, :] == 1])
        result = p.solve()
        self.assertAlmostEqual(result, 3)
        self.assertItemsAlmostEqual(self.C.value[0:3:2, 1], [1, 2])

        p = Problem(cvx.Maximize(cvx.sum((self.C[0:2, :] + self.A)[:, 0:2])),
                    [self.C[1:3, :] <= 2, self.C[0, :] == 1,
        ...
        p = Problem(cvx.Maximize([[3], [4]]*(self.C[0:2, :] + self.A)[:, 0]),
                    [self.C[1:3, :] <= 2, self.C[0, :] == 1,
        ...
        # Transpose of slice.
        p = Problem(cvx.Maximize(cvx.sum(self.C)), [self.C[1:3, :].T <= 2, self.C[0, :].T == 1])
        result = p.solve()
        self.assertAlmostEqual(result, 10)
        self.assertItemsAlmostEqual(self.C.value, 2*[1, 2, 2])
    ...
    def test_transpose(self):
        ...
        p = Problem(cvx.Minimize(cvx.sum(self.C)), constraints)
        result2 = p.solve()
        self.assertAlmostEqual(result, result2)
        self.assertItemsAlmostEqual(self.C.value, value)

        p = Problem(cvx.Minimize(self.A[0, 1] - self.A.T[1, 0]),
                    [self.A == [[1, 2], [3, 4]]])
        result = p.solve()
        self.assertAlmostEqual(result, 0)

        exp = (-self.x).T
        p = Problem(cvx.Minimize(cvx.sum(self.x)), [(-self.x).T <= 1])
        result = p.solve()
        self.assertAlmostEqual(result, -2)
        ...
        # Slice of transpose.
        p = Problem(cvx.Maximize(cvx.sum(self.C)), [self.C.T[:, 1:3] <= 2, self.C.T[:, 0] == 1])
        result = p.solve()
        self.assertAlmostEqual(result, 10)
        self.assertItemsAlmostEqual(self.C.value, 2*[1, 2, 2])
    ...
    def test_multiplication_on_left(self):
        ...
        p = Problem(cvx.Minimize(self.a*2), [self.a >= 2])
        result = p.solve()
        self.assertAlmostEqual(result, 4)

        p = Problem(cvx.Minimize(self.x.T*c), [self.x >= 2])
        result = p.solve()
        self.assertAlmostEqual(result, 6)

        p = Problem(cvx.Minimize((self.x.T + self.z.T)*c),
                    [self.x >= 2, self.z >= 1])
        result = p.solve()
        self.assertAlmostEqual(result, 9)
    ...
    def test_redundant_constraints(self):
        ...
        obj = cvx.Minimize(cvx.sum(cvx.square(self.x)))
    ...
    def test_sdp_symmetry(self):
        ...
        p = Problem(cvx.Minimize(cvx.lambda_max(self.A)), [self.A >= 2])
        result = p.solve()
        self.assertItemsAlmostEqual(self.A.value, self.A.value.T, places=3)

        p = Problem(cvx.Minimize(cvx.lambda_max(self.A)), [self.A == [[1, 2], [3, 4]]])
        result = p.solve()
        self.assertEqual(p.status, s.INFEASIBLE)
    ...
    def test_sdp(self):
        ...
        obj = cvx.Maximize(self.A[1, 0] - self.A[0, 1])
        p = Problem(obj, [cvx.lambda_max(self.A) <= 100,
                          self.A[0, 0] == 2,
                          self.A[1, 1] == 2,
                          self.A[1, 0] == 2])
        result = p.solve()
    ...
    def test_div(self):
        ...
        """
        obj = cvx.Minimize(cvx.norm_inf(self.A/5))
    ...
    def test_reshape(self):
        ...
        """
        # Test on scalars.
        self.assertEqual(cvx.reshape(1, (1, 1)).value, 1)
        ...
        c = [1, 2, 3, 4]
        expr = cvx.reshape(self.A, (4, 1))
        obj = cvx.Minimize(expr.T*c)
        constraints = [self.A == [[-1, -2], [3, 4]]]
        prob = Problem(obj, constraints)
        result = prob.solve()
        self.assertAlmostEqual(result, 20)
        self.assertItemsAlmostEqual(expr.value, [-1, -2, 3, 4])
        self.assertItemsAlmostEqual(cvx.reshape(expr, (2, 2)).value, [-1, -2, 3, 4])
        ...
        expr = cvx.reshape(c*self.a, (4, 1))
        obj = cvx.Minimize(expr.T*[1, 2, 3, 4])
        prob = Problem(obj, [self.a == 2])
        result = prob.solve()
        self.assertAlmostEqual(result, -6)
        self.assertItemsAlmostEqual(expr.value, 2*c)
    ...
    def test_diag_prob(self):
        ...
        """
        C = Variable((3, 3))
        obj = cvx.Maximize(C[0, 2])
        constraints = [cvx.diag(C) == 1,
                       C[0, 1] == 0.6,
                       C[1, 2] == -0.3,
    ...
    def test_parameter_expressions(self):
        ...
        # set up the problem
        obj = cvx.abs(x - 1)
        prob = Problem(cvx.Minimize(obj), [g == 0])
        prob.solve()
        x0.value = 1
        prob.solve()
        ...
        # Test multiplication.
        prob = Problem(cvx.Minimize(x0*x), [x == 1])
        x0.value = 2
        prob.solve()
        x0.value = 1
        prob.solve()
    ...
    def test_geo_mean(self):
        ...
        x = Variable(2)
        cost = cvx.geo_mean(x)
        prob = Problem(cvx.Maximize(cost), [x <= 1])
        prob.solve()
        self.assertAlmostEqual(prob.value, 1)

        prob = Problem(cvx.Maximize(cost), [cvx.sum(x) <= 1])
        prob.solve()
        self.assertItemsAlmostEqual(x.value, [.5, .5])

        x = Variable((3, 3))
        self.assertRaises(ValueError, cvx.geo_mean, x)
        ...
        x = Variable(5)
        prob = Problem(cvx.Maximize(cvx.geo_mean(x, p)), [cvx.sum(x) <= 1])
        prob.solve()
        x = np.array(x.value).flatten()
        x_true = p/sum(p)

        self.assertTrue(np.allclose(prob.value, cvx.geo_mean(list(x), p).value))
        ...
        x = Variable(5)
        prob = Problem(cvx.Maximize(cvx.geo_mean(x, p)), [cvx.norm(x) <= 1])
        prob.solve()
        x = np.array(x.value).flatten()
        ...
        Problem(cvx.Maximize(cvx.geo_mean(x)), [x <= 1]).solve()
    ...
    def test_power(self):
        x = Variable()
        prob = Problem(cvx.Minimize(cvx.power(x, 1.7) + cvx.power(x, -2.3) - cvx.power(x, .45)))
        prob.solve()
        x = x.value
    ...
    def test_multiply_by_scalar(self):
        ...
        a = cvx.multiply(dy[0:T], loglambda)  # size(Tx1)
        b1 = cvx.exp(loglambda)
        b2 = cvx.multiply(delta, b1)
        cost = -a + b1

        cost = -a + b2  # size (Tx1)
        prob = Problem(cvx.Minimize(cvx.sum(cost)))
        prob.solve(solver=s.SCS)

        obj = cvx.Minimize(cvx.sum(cvx.multiply(2, self.x)))
