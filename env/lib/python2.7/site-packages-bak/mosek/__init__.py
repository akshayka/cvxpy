...
class SCoptException(MSKException): pass
...
def loadmosek(libbasename):
    ...
    # location of libs if this is installed with conda
    dlldirs.append(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))))
    library = None
...
__library__,__dlldir__ = loadmosek("mosekxx8_0")
...
def __load_scopt__():
    ...
    try:
        if __scopt__ is None:
            ...
            __scopt__.MSK_scattach.argtypes = [ ctypes.c_void_p, ctypes.c_void_p ]
            __scopt__.MSK_scdetach.restype  = ctypes.c_int
            __scopt__.MSK_scdetach.argtypes = [ ctypes.c_void_p ]
...
class Env:
  ...
  def __del__(self):
    try:
      if self.__nativep is not None:
        self.checkinall()
        res = self.__library.MSK_XX_deleteenv(ctypes.byref(self.__nativep))
  ...
  def computesparsecholesky(self,multithread_,ordermethod_,tolsingular_,anzc_,aptrc_,asubc_,avalc_):
    ...
    computesparsecholesky(self,multithread_,ordermethod_,tolsingular_,anzc_,aptrc_,asubc_,avalc_)
      ...
      ordermethod: int. If nonzero, then a sparsity preservering ordering will be employed.
      tolsingular: double. A positive parameter controling when a pivot is declared zero.
      ...
      asubc: array of int. Row subscripts in each column stored in increasing order
      avalc: array of double. Values stores column wise.
    returns: lensubnval
  ...
  def sparsetriangularsolvedense(self,transposed_,lnzc_,lptrc_,lsubc_,lvalc_,b_):
    ...
    sparsetriangularsolvedense(self,transposed_,lnzc_,lptrc_,lsubc_,lvalc_,b_)
      transposed: mosek.transpose. Controls whether the solve is with L or the tranposed L. 
      ...
      b: array of double. The right-hand side of linear equation system to be solved.
    """
    n_ = None
    if n_ is None:
      n_ = len(b_)
...
class Task:
  ...
  def __del__(self):
      try:
          if self.__nativep is not None:
              self.removeSCeval()        
              res = self.__library.MSK_XX_deletetask(ctypes.byref(self.__nativep))
              del self.__library
              del self.__schandle
  ...
  def writeSC(self,scfile,taskfile):
      if self.__schandle is not None:
          if not __scopt__.MSK_scwritefile(self.__nativep, self.__schandle, scfile, taskfile):
  ...
  def putSCeval(self,
    ...
    if (    opro  is not None 
        ...
        try:
            ...
            _oprfo = numpy.array(oprfo,float)
            _oprgo = numpy.array(oprgo,float)
            _oprho = numpy.array(oprho,float)
  ...
  def getacol(self,j_,subj_,valj_):
    ...
    getacol(self,j_,subj_,valj_)
      ...
      subj: array of int. Index of the non-zeros in the row obtained. 
  ...
  def getcone(self,k_,submem_):
    ...
    _submem_minlength = self.getcone((k_),None)[2]
    if self.getcone((k_),None)[2] > 0 and submem_ is not None and len(submem_) != self.getcone((k_),None)[2]:
      raise ValueError("Array argument submem is not long enough: Is %d, expected %d" % (len(submem_),self.getcone((k_),None)[2]))
  ...
  def getmaxnumbarvar(self):
    ...
    returns: maxnumbarvar
      maxnumbarvar: int. Maximum number of symmetric matrix variables currently that is reservered room for. 
    """
  ...
  def getprimalsolutionnorms(self,whichsol_):
    ...
    returns: nrmxc,nrmxx,nrmbarx
      nrmxc: double. The norm of xc vector. 
      nrmxx: double. The norm of xx vector. 
      nrmbarx: double. The norm of barx vector. 
    """
  ...
  def putbaraij(self,i_,j_,sub_,weights_):
    ...
    putbaraij(self,i_,j_,sub_,weights_)
      ...
      sub: array of long. See argument weights for an explanation. 
  ...
  def getbaraidx(self,idx_,sub_,weights_):
    ...
    getbaraidx(self,idx_,sub_,weights_)
      idx: long. Position of the element in the vectorized form. 
      sub: array of long. A list indexes   of the elements from symmetric matrix storage that appears in the weighted sum. 
  ...
  def getbarcblocktriplet(self,subj_,subk_,subl_,valijkl_):
    ...
    getbarcblocktriplet(self,subj_,subk_,subl_,valijkl_)
      ...
      subk: array of int. Block row index. 
      subl: array of int. Block column index. 
      valijkl: array of double. A list indexes of the elements from symmetric matrix storage that appears in the weighted sum. 
    returns: num
      num: long. Number of elements in the block triplet form. 
    """
  ...
  def optimizermt(self,server_,port_):
    ...
    optimizermt(self,server_,port_)
      server: str|unicode. Name or IP address of the server 
  ...
  def asyncpoll(self,server_,port_,token_):
    ...
    returns: respavailable,resp,trm
      ...
      resp: mosek.rescode. Is either the response code from from the remote solver.  
